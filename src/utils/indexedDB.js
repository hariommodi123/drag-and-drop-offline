...
const DB_VERSION = 11; // Incremented to add updatedAt and _id indexes for orders store
...
const validateCustomer = (customer) => {
  ...
  if (customer.dueAmount !== undefined && customer.dueAmount !== null) {
    if (typeof customer.dueAmount !== 'number' || customer.dueAmount < 0) {
...
const validateTransaction = (transaction) => {
  ...
  // Transaction should only be used for plan purchases
  const validTypes = ['plan_purchase'];
  if (!transaction.type || !validTypes.includes(transaction.type)) {
    errors.push(`Transaction type must be 'plan_purchase' (use Order model for sales)`);
  }
...
export const addItem = async (storeName, item, skipValidation = false) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onerror = () => {
      // Check if error is due to duplicate key (ID already exists)
      if (request.error && request.error.name === 'ConstraintError') {
        ...
        const updateRequest = store.put(itemWithSync);
