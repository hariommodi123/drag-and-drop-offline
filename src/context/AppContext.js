...
const getInitialState = () => {
  ...
  upiId: settingsState.upiId || authState.currentUser?.upiId || '',
    ...
    case ActionTypes.UPDATE_CUSTOMER:
      ...
      if (syncService.isOnline()) {
            syncService.syncAll(getStoreFunctions).catch(() => {
              if (window.showToast) {
                window.showToast('Customer update saved. We\'ll sync it automatically soon.', 'info');
              }
            });
          }
        })
      ...
      updateInIndexedDB(STORES.products, updatedProduct)
        .then(() => {
          ...
          if (!suppressProductSync && syncService.isOnline()) {
            syncService.syncAll(getStoreFunctions).catch(() => {
              if (window.showToast) {
                window.showToast('Product update saved. We\'ll sync it automatically soon.', 'info');
              }
            });
          }
        })
            ...
            setTimeout(async () => {
              ...
              try {
                ...
                if (createResult.success) {
                  ...
                  if (window.showToast) {
                    window.showToast('Order saved. We\'ll sync it automatically.', 'info');
                  }
                }
              } catch (error) {
      ...
      updateInIndexedDB(STORES.orders, updatedOrder)
        .then(() => {
          ...
          if (!updatedOrder.isSynced && syncService.isOnline()) {
            syncService.syncAll(getStoreFunctions).catch(() => {
              if (window.showToast) {
                window.showToast('Order update saved. We\'ll sync it automatically soon.', 'info');
              }
            });
          }
        })
        .catch(err => {
          ...
          if (window.showToast) {
            window.showToast('Couldn\'t update order. Please try again.', 'error');
          }
        });
      ...
      updateInIndexedDB(STORES.purchaseOrders, updatedPO)
        .then(() => {
          ...
          if (syncService.isOnline()) {
            syncService.syncAll(getStoreFunctions).catch(() => {
              if (window.showToast) {
                window.showToast('Purchase order update saved. We\'ll sync it automatically soon.', 'info');
              }
            });
          }
        })
      ...
      updateInIndexedDB(STORES.transactions, updatedTransaction)
        .then(() => {
          ...
          if (syncService.isOnline()) {
            syncService.syncAll(getStoreFunctions).catch(() => {
              if (window.showToast) {
                window.showToast('Transaction update saved. We\'ll sync it automatically soon.', 'info');
              }
            });
          }
        })
      ...
      try {
        ...
        if (isOnlineStatus) {
          ...
          if (combinedPlanDetails) {
            ...
            dispatch({ type: ActionTypes.SET_CURRENT_PLAN_DETAILS, payload: null });
          }
        }
      } catch (error) {
