  ...
  const handlePlanSelect = async (planId) => {
    ...
    if (selectedPlan?.userHasThisPlan) {
      try {
        ...
        if (result.success) {
          ...
          window.showToast(result.error || 'Failed to switch plan', 'error');
        }
      } catch (err) {
        console.error('Error switching plan:', err);
        window.showToast(err.message || 'Failed to switch plan', 'error');
      } finally {
    ...
    if (planPrice === 0) {
      try {
        ...
        if (result.success) {
          ...
          window.showToast(result.error || 'Failed to upgrade plan', 'error');
        }
      } catch (err) {
        console.error('Error upgrading plan:', err);
        window.showToast(err.message || 'Failed to upgrade plan', 'error');
      } finally {
        setUpgradingPlanId(null);
      }
      return;
    ...
    try {
      ...
      if (!orderResult.success) {
        window.showToast(orderResult.error || 'Failed to create payment order', 'error');
        setUpgradingPlanId(null);
        return;
      }
      ...
      const { orderId, amount, currency, key, planOrderId } = responseData;
      ...
      if (!orderId || !amount || !currency) {
        console.error('Missing payment details:', { orderId, amount, currency, key, planOrderId });
        ...
        handler: async function (response) {
          try {
            ...
            if (verifyResult.success) {
              ...
              window.showToast(verifyResult.error || 'Payment verification failed', 'error');
            }
          } catch (verifyError) {
      ...
      window.showToast(err.message || 'Failed to initiate payment', 'error');
      setUpgradingPlanId(null);
    }
  };
      ...
      <div className="bg-gradient-to-r from-green-500 to-blue-500 rounded-xl p-6 text-white">
        <div className="flex items-center justify-between">
          <div>
            ...
            {activePlanOrdersCount > 1 && (
              <p className="text-green-100 text-sm mt-1">
                You have {activePlanOrdersCount} active plan orders
              </p>
            )}
            <p className="text-green-100 text-sm mt-2">
