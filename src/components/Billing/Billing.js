...
const Billing = () => {
  ...
  const generateBill = () => {
    ...
    const order = {
      ...
      customerId: customerId, // Optional (can be null for walk-in customers)
      paymentMethod: paymentMethod, // Required: "cash", "card", "upi", "due", "credit"
      items: orderItems, // Required: Array of { name, sellingPrice, costPrice, quantity, unit }
      totalAmount: normalizedTotal, // Required - normalized to 2 decimal places
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString(),
      isSynced: false // Will be synced to MongoDB
    };

    console.log('🎯 Creating Order (MongoDB schema):', order);
    console.log('🎯 Order items:', orderItems);
    console.log('🎯 Seller ID:', sellerId);
    console.log('🎯 Customer ID:', customerId);

    // Dispatch ADD_ORDER action (not ADD_TRANSACTION - Transaction is only for plan purchases)
    ...
    const bill = {
      ...
      items: billItems, // Use original billItems for UI display
      subtotal: subtotal,
    ...
    // Update quantity for each item with proper unit conversion
    billItems.forEach(item => {
      ...
      if (product) {
        ...
        const currentQuantity = product.quantity || product.stock || 0; // Support migration
    ...
    if (paymentMethod === 'due' || paymentMethod === 'credit') {
      ...
      if (customer) {
        const updatedCustomer = {
          ...customer,
          balanceDue: (customer.balanceDue || customer.dueAmount || 0) + total,
          dueAmount: (customer.dueAmount || customer.balanceDue || 0) + total, // Use dueAmount for database
          // Update mobile if provided and different
          mobileNumber: (useCustomName && customerMobileNumber) ? customerMobileNumber : (customer.mobileNumber || customer.phone || '')
        };
        ...
        const newCustomer = {
          ...
          name: customerName,
          balanceDue: total,
          dueAmount: total, // Use dueAmount for database
          mobileNumber: customerMobileNumber || '', // Use mobileNumber instead of phone
          email: '',
          address: '',
          createdAt: new Date().toISOString()
        };
    ...
    // Add activity for order creation
    dispatch({ type: 'ADD_ACTIVITY', payload: {
      id: Date.now().toString(),
      message: `Order created for ${customerName} - ₹${total.toFixed(2)} (${paymentMethod})`,
    ...
    // Reset flag after a short delay to allow order creation to complete
    setTimeout(() => {
      isGeneratingBill.current = false;
    }, 2000); // 2 seconds should be enough for the API call to complete
