...
const Billing = () => {
  ...
  const isGeneratingBill = useRef(false); // Prevent multiple simultaneous bill generations
  ...
  const openWhatsAppInvoice = (bill, mobile) => {
    ...
    const baseNumber = sanitized.length === 12 && sanitized.startsWith('91') ? sanitized.slice(2) : sanitized;
    const isValidIndianNumber = /^[6-9]\d{9}$/.test(baseNumber);
    if (!isValidIndianNumber) {
      showToast('Mobile number is incorrect. Please enter a valid 10-digit number starting with 6-9.', 'error');
      return;
    }

    const withCountryCode = `91${baseNumber}`;
  ...
  // Always keep focus on barcode input on mount and after touch events
  useEffect(() => {
    const focusBarcode = () => {
    ...
    focusBarcode();
    // Focus after any touch on the page
    document.addEventListener('touchstart', focusBarcode, { passive: true });
    // Also focus after visibility change (e.g., returning to tab)
    const onVisibility = () => {
      if (!document.hidden) focusBarcode();
    };
    document.addEventListener('visibilitychange', onVisibility);
    return () => {
      document.removeEventListener('touchstart', focusBarcode);
  ...
  const handleAddWithQuantity = (product, quantity, unit) => {
    ...
    if (existingItem) {
      ...
      setBillItems(prev => [...prev, {
        ...
        maxQuantity: product.quantity || product.stock || 0, // Store original quantity for reference
    ...
    billItemsSnapshot.forEach(item => {
      ...
      if (product) {
        ...
        const billedQuantityInProductUnit = convertFromBaseUnit(
          convertToBaseUnit(item.quantity, item.unit || productUnit),
          productUnit
        );
        ...
        const updatedProduct = {
          ...product,
          quantity: Math.max(0, currentQuantity - billedQuantityInProductUnit)
        };
  ...
  const generateBill = () => {
    ...
    if (pendingOrder) {
      showToast(
        state.currentLanguage === 'hi'
          ? 'कृपया नया बिल बनाने से पहले लंबित UPI भुगतान पूरा करें।'
          : 'Please complete the pending UPI payment before creating a new bill.',
        'warning'
      );
      return;
    }
    ...
    const sanitizedMobile = sanitizeMobile(effectiveMobile);
    ...
    const orderItems = billItems.map(billItem => {
      const product = state.products.find(p => p.id === billItem.id);
      const costPrice = product ? (product.costPrice || product.unitPrice || 0) : 0;
      const sellingPrice = billItem.price || 0;
      
      return {
      productId: product?._id || billItem.productId || null,
        name: billItem.name || product?.name || '',
        sellingPrice: typeof sellingPrice === 'number' ? sellingPrice : parseFloat(sellingPrice) || 0,
        costPrice: typeof costPrice === 'number' ? costPrice : parseFloat(costPrice) || 0,
        quantity: typeof billItem.quantity === 'number' ? billItem.quantity : parseFloat(billItem.quantity) || 1,
        unit: billItem.unit || product?.unit || 'pcs'
      };
    });
    ...
    // Create bill object for UI compatibility (used in UPI payment modal, PDF, QR code, etc.)
    const bill = {
      id: order.id,
    ...
    if (paymentMethod === 'upi') {
      ...
      if (!sellerUpiId) {
        showToast('Please add your business UPI ID in Settings before accepting UPI payments.', 'error');
        isGeneratingBill.current = false;
        return;
      }

      if (pendingOrder) {
        showToast('Please complete the pending UPI payment before creating a new bill.', 'warning');
        isGeneratingBill.current = false;
        return;
      }
  ...
  const handlePaymentReceived = (paymentSummary) => {
    if (!pendingOrder) {
      showToast('No pending UPI payment to confirm.', 'warning');
      setShowUPIPayment(false);
      setCurrentBill(null);
      return;
    }
    ...
    if (success) {
      dispatch({
        ...
        payload: {
          id: Date.now().toString(),
          message: `UPI payment received for Bill #${pendingOrder.bill.id} - ₹${pendingOrder.bill.total.toFixed(2)}${paymentSummary?.transactionId ? ` (Txn: ${paymentSummary.transactionId})` : ''}`,
          timestamp: new Date().toISOString(),
  ...
  const makePayment = () => {
    ...
    try {
      ...
      showToast(`Order created successfully! Stock reduced for ${billItems.length} item(s).`, 'success');
    } catch (error) {
  ...
  return (
    <div className="space-y-8 fade-in-up">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-3xl font-bold text-black">{getTranslation('billingSystem', state.currentLanguage)}</h2>
        ...
        <div className="mt-4 sm:mt-0 flex flex-wrap gap-2 sm:gap-3">
          ...
          <button
            onClick={(e) => {
              console.log('🔘 BUTTON CLICKED - Confirm Order button pressed!');
              console.log('Event:', e);
              console.log('Bill items length:', billItems.length);
              console.log('Button disabled?', billItems.length === 0);
              console.log('makePayment function:', makePayment);
              console.log('makePayment type:', typeof makePayment);
              
              if (billItems.length === 0) {
                console.log('⚠️ Button is disabled - no bill items');
                return;
              }
              
              try {
                console.log('🚀 Calling makePayment function...');
                makePayment();
                console.log('✅ makePayment function completed');
              } catch (error) {
            ...
            className="btn-primary flex items-center bg-primary-500 hover:bg-primary-600 text-sm"
            disabled={billItems.length === 0}
            style={{ cursor: billItems.length === 0 ? 'not-allowed' : 'pointer' }}
          >
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Select Customer {(paymentMethod !== 'due' && paymentMethod !== 'credit') && (
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="input-field"
                    />
                    {(paymentMethod === 'due' || paymentMethod === 'credit') && !billingMobile && (
                    ...
                    <div className="mt-2 flex items-center space-x-2">
                      ...
                      />
                      <label htmlFor="send-whatsapp-invoice" className="text-sm text-gray-700">
          ...
          <div className="card">
            ...
            <div className="space-y-3 mb-4">
              ...
              <div className="relative">
                <ScanLine className="absolute left-3 top-1/2 transform -translate-y-1/2 h-5 w-5 text-primary-600" />
                <input
                  type="text"
                ...
                <button
                  ...
                  className="absolute right-2 top-1/2 transform -translate-y-1/2 p-2 bg-primary-500 text-white rounded-lg hover:bg-primary-600 transition-colors"
              ...
              {filteredProducts.map(product => (
                ...
                >
                  <div>
                    ...
                    <p className="text-sm text-gray-600">₹{(product.sellingPrice || product.costPrice || 0).toFixed(2)} per {product.quantityUnit || product.unit || 'pcs'} • Quantity: {product.quantity || product.stock || 0}</p>
                  </div>
                  <button
          ...
          <div className="card sticky top-4">
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-primary-600">-₹{discountAmount.toFixed(2)}</span>
              </div>
            ...
            <div className="mt-6 space-y-4">
              ...
              <button
                ...
                disabled={
                  ...
                  billItems.length === 0 ||
                  // Require customer details only for due/credit payments
        ...
        {showBarcodeMachine && (
          <BarcodeMachine
            onScan={(barcode) => {
              ...
              handleBarcodeScan(barcode);
              setShowBarcodeMachine(false);
              // Refocus barcode input after closing scanner
              setTimeout(() => {
