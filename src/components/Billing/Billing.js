...
import { calculatePriceWithUnitConversion, checkStockAvailability, convertToBaseUnit, convertFromBaseUnit, getBaseUnit } from '../../utils/unitConversion';
...
const Billing = () => {
  ...
  const handleSaveUpiId = () => {
    ...
    showToast('UPI ID saved for future UPI payments.', 'success');
  };
  ...
  const handleAddWithQuantity = (product, quantity, unit) => {
    ...
    if (existingItemIndex >= 0) {
      ...
      const newItem = buildBillItem(product, quantity, unit, stockCheck.baseUnit);
  ...
  const updateQuantity = (productId, quantity) => {
    if (quantity <= 0) {
      ...
      setBillItems(prev => prev.map(existing =>
        existing.id === productId ? { ...existing, quantity } : existing
      ));
    }
  };
  ...
  const finalizeOrder = ({
    ...
    billItemsSnapshot.forEach(item => {
      ...
      if (product) {
        ...
        const billedQuantityInBaseUnit = convertToBaseUnit(
          item.quantity,
  ...
  const generateBill = () => {
    ...
    const orderItems = billItems.map(billItem => {
      ...
      const normalizedQuantity = typeof billItem.quantity === 'number'
        ? billItem.quantity
        : parseFloat(billItem.quantity) || 1;
      const roundedQuantity = Number.isFinite(normalizedQuantity) ? normalizedQuantity : 0;
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                {paymentMethod === 'upi' && !sellerUpiId && (
                  <div className="mt-3 rounded-lg border border-primary-200 bg-primary-50 p-4 space-y-3">
                    <div className="text-sm font-semibold text-primary-800">Add your business UPI ID</div>
                    <p className="text-xs text-primary-700">
                      Save your UPI ID once to accept UPI payments and auto-fill future bills.
                    </p>
                ...
                {paymentMethod === 'upi' && sellerUpiId && (
                  <div className="mt-2 text-xs text-primary-600">
                    UPI payments will use <span className="font-semibold">{sellerUpiId}</span>.
                  </div>
                )}
              </div>
            </div>
              ...
              <div className="relative">
                ...
                <div className="absolute right-2 top-1/2 flex -translate-y-1/2 items-center">
                  <button
                    ...
                    className="p-2 bg-white border border-primary-500 text-primary-600 rounded-lg hover:bg-primary-50 transition-colors"
