...
const Dashboard = () => {
  ...
  const calculateProfitFromOrderItems = (orders) => {
    return orders.reduce((totalProfit, order) => {
      ...
      const orderProfit = order.items.reduce((orderItemProfit, item) => {
        const sellingPrice = typeof item.sellingPrice === 'number' ? item.sellingPrice : parseFloat(item.sellingPrice) || 0;
  ...
  const stats = [
    ...
    },
    {
      ...
      value: formatNumber(rangeProfit),
      icon: TrendingUp,
      color: 'bg-cyan-500',
            ...
            <div className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
              ...
              <div className="text-right">
                <p className="text-xl font-bold text-black">
                  {Math.round(monthlyOrders.length / 30) || 0}
                </p>
              </div>
            </div>
            <div className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
