...
const Sidebar = ({ onClose }) => {
  ...
  const handleNavigation = (view) => {
    ...
    if (!isModuleUnlocked(view, state.currentPlan, state.currentPlanDetails)) {
    ...
    <div className="flex flex-col h-full">
      ...
      <ul className="flex-1 flex flex-col py-4 space-y-2 overflow-y-auto">
        {navigation.map((item) => {
          ...
          const isUnlocked = isModuleUnlocked(item.href, state.currentPlan, state.currentPlanDetails);
          ...
          <button
            ...
            className={`flex items-center px-4 py-3 mx-3 w-full text-left rounded-lg transition-all duration-200 ${
              state.currentView === 'settings' 
                ? 'bg-white text-black shadow-lg font-semibold' 
                : isModuleUnlocked('settings', state.currentPlan, state.currentPlanDetails)
                ? 'text-gray-300 hover:bg-gray-900 hover:text-white'
                : 'text-gray-700 cursor-not-allowed'
            }`}
            disabled={!isModuleUnlocked('settings', state.currentPlan, state.currentPlanDetails)}
          >
            <Settings className={`w-6 h-6 ${state.currentView === 'settings' ? 'text-black' : isModuleUnlocked('settings', state.currentPlan, state.currentPlanDetails) ? 'text-gray-300' : 'text-gray-700'}`} />
